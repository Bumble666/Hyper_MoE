...
from transformers.models.adapter_generators import ParameterGenerator

from ..ad_layers import AdapterLayer
...
class SwitchTransformersTop1Router(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple:
        ...
        length = router_logits.size(1)
        exp_num = router_logits.size(2)
        gates = None
        gates_top1 = None

        expert_index = torch.argmax(router_probs, dim=-1)  # [batch, length]
        expert_index = torch.nn.functional.one_hot(expert_index,
                                                   num_classes=self.num_experts)  # [batch, length, num_exps]
        if self.use_hypernet:
            gates_top1 = expert_index
            gates = torch.ones_like(router_logits, requires_grad=True, dtype=router_logits.dtype).to(
                hidden_states.device) - expert_index
            gates = (gates / gates.sum(dim=-1, keepdim=True)).flatten(0, 1)  # bs*length, 8
        ...
        router_probs = torch.max(router_probs, dim=-1).values.unsqueeze(-1)  # (batch, length,1)
        return expert_index, router_probs, router_logits, gates, gates_top1
...
class SwitchTransformersDenseActDense(nn.Module):
    ...
    def forward(self, hidden_states, param_gen=None):
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def __init__(self, layer_idx, config: SwitchTransformersConfig,
        ...
        if config.use_hypernet:
            self.adapter_layer = AdapterLayer(config, self.input_dim, self.input_dim)
        else:
            self.adapter_layer = None
        self.unselected_exp = (config.num_experts - 1)
        self.num_experts = config.num_experts
        ...
        # self.index = torch.arange(0, self.num_experts, dtype=torch.long, device=self.router.device)

        if self.expert_embedding_mode:
            self.n_experts_embedding = nn.Embedding(config.num_experts, config.experts_embedding_dim)
    ...
    def forward(self, hidden_states, param_gen=None):
        ...
        router_mask, router_probs, router_logits, gates_out, gates_top1 = self.router(hidden_states)
        expert_index = torch.argmax(router_mask, dim=-1)
        # mask:[batch, length, num_exps]
        # The routers introduced might not always map all the tokens, to a router, which means that some hidden states
        # can be unchanged from one layer to another. That is why the hidden states are cloned before updating only the seleced ones.

        if gates_out is not None:
            index_out = torch.nonzero(gates_out)[:, -1:].flatten()  # batch*l*unselected_exp, em
            index_out_top1 = torch.nonzero(gates_top1)[:, -1:].flatten()
            gates_out = gates_out[gates_out != 0]
        ...
        if self.expert_embedding_mode:
            ...
            embedding_input = torch.sum(
                self.n_experts_embedding(index_out).view(hidden_states.size(0), hidden_states.size(1),
                                                         self.unselected_exp, -1), dim=-2)
            self.apply_params_to_adapters(hidden_states.size(0), hidden_states.size(1),
                                          param_gen(self.embedding_process(embedding_input), self.layer_idx))
            out_adapter = self.adapter_layer(res)
            out = out_adapter + hidden_states
...
class SwitchTransformersLayerFF(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig, is_sparse=False, layer_idx=None):
        ...
        if not self.is_sparse:
            ...
            self.mlp = SwitchTransformersSparseMLP(layer_idx, config)

        self.layer_norm = SwitchTransformersLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)

    def forward(self, hidden_states, output_router_logits, param_gen):
        forwarded_states = self.layer_norm(hidden_states)

        forwarded_states = self.mlp(forwarded_states, param_gen)
...
class SwitchTransformersBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, output_router_logits, param_gen)
...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.gradient_checkpointing = False
        if config.use_hypernet:
            self.param_gen = ParameterGenerator(config, config.d_model, config.d_model)
        else:
            self.param_gen = None
