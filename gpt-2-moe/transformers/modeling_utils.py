...
moe_note_dict = {
            'bert': ('.output.dense.', '.intermediate.dense.'), 
            'bart': ('.fc1.', '.fc2.'), 
            'gpt2': ('.c_proj.', '.c_fc.'), 
            't5': ('.wi_0.', '.wi_1.', '.wo.', '.wi.'), 
            }    
...
def _load_state_dict_into_model(model_to_load, state_dict, start_prefix):
    ...
    if hasattr(model_to_load.config, 'seed'):
        torch.manual_seed(model_to_load.config.seed) # todo maintain the consistence between single model and moe.
    # todo load the weight in the experts
    if hasattr(model_to_load.config, 'use_moe') and model_to_load.config.use_moe in ['MoE', 'Conv1D_MoE']:
        load_MoE(model_to_load, state_dict)

    if hasattr(model_to_load.config, 'use_moe') and model_to_load.config.use_moe in ['MEO', 'Conv1D_MEO']:
        load_MEO(model_to_load, state_dict)
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _load_pretrained_model(
        ...
        moe_notes = moe_note_dict[model_to_load.config.model_type]
        unexpected_keys = [key for key in unexpected_keys if not any(note in key for note in moe_notes)]
        missing_keys = [key for key in missing_keys if not 'experts' in key]
        if len(unexpected_keys) > 0:
            logger.warning(
...
def load_MoE(model, state_dict):
    
    model_type = model.config.model_type
    loaded_keys = list(state_dict.keys())
    moe_notes = moe_note_dict[model_type]

    for key in loaded_keys:
        for moe_note in moe_notes: 
            if moe_note in key and 'attention' not in key and 'attn' not in key: 
                values = state_dict.pop(key, None)
                if values is None:
                    continue
                if model_type in ['gpt2']:
                    for expert in range(model.config.n_experts):
                        moe_key = key.replace(moe_note, '.experts.' + f"expert_{expert}" + moe_note)
                        state_dict[moe_key] = values
                        #state_dict[moe_key] = values.transpose(-2, -1) if 'bias' not in moe_key else values
                elif model_type in ['bert']:
                    for expert in range(model.config.n_experts):
                        moe_key = key.replace(moe_note, '.mlp.' + 'experts.' + f"expert_{expert}" + moe_note)
                        state_dict[moe_key] = values
                        #print(key)
                        #print(moe_key)
                        #print("---------------------")
                    #
                    # if model_type in ['bart', 'bert', 't5']:
                    #     state_dict[moe_key] = values
                    # else:
                    #     state_dict[moe_key] = values.transpose(-2, -1) if 'bias' not in moe_key else values

def load_MEO(model, state_dict):
        ...
        for moe_note in moe_notes:
            if moe_note in key and 'attention' not in key and 'attn' not in key:
                ...
                if values is None:
                    continue
                if model_type in ['bart', 'bert', 't5']:
                    state_dict[key] = values.unsqueeze(0).repeat(model.config.n_experts, 1) if 'bias' in key \
                                else values.unsqueeze(0).repeat(model.config.n_experts, 1, 1)
                else:
                    state_dict[key] = values.unsqueeze(0).repeat(model.config.n_experts, 1) if 'bias' in key \
                                else values.transpose(-2, -1).unsqueeze(0).repeat(model.config.n_experts, 1, 1)
